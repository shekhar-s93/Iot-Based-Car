...
const unsigned long DISCONNECT_TIMEOUT = 8000; // 3 seconds
...
void updateUltrasonic() {
  ...
  unsigned long d = pulseIn(ECHO_PIN, HIGH, 12000); // shorter timeout
...
void handleManual() {
  ...
  else if (backCmd && leftCmd)      driveLeft(carSpeed);
  else if (backCmd && rightCmd)     driveRight(carSpeed);
...
void handleFollow() {
  ...
  if (d < 14) followState = -1;
  else if (d > 30) followState = 1;
  else followState = 0;
