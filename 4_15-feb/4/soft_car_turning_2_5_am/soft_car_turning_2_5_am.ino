...
float steeringRatio = 0.3;   // 0.1 = sharp , 0.5 = smooth
float pivotRatio = 0.25;   // inner wheel reverse strength
...
void handleManual() {
  ...
  if (forwardCmd) {
    ...
    if (leftCmd) {
      leftSpeed  = -carSpeed * pivotRatio;   // reverse inner wheel
      rightSpeed =  carSpeed;                // outer fast
    }
    else if (rightCmd) {
      rightSpeed = -carSpeed * pivotRatio;
      leftSpeed  =  carSpeed;
    }
    else {
      leftSpeed = carSpeed;
...
void handleObstacle() {
  ...
  if (d < 30) {
    if (turnLeftNext)
      applyMotorSpeed(carSpeed - turnStrength, carSpeed);
    else
      applyMotorSpeed(carSpeed, carSpeed - turnStrength);
    return;
  }
