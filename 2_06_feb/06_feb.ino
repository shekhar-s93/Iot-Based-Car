...
const float OBSTACLE_DIST_CM = 30.0;
...
int readDistanceSafe() {
  if (cachedDistance < 0) return 0;
  return (int)cachedDistance;
}
...
// ================= Servo Scan =================   // (optional comment)
void scanAndDecideDirection() {

  // If obstacle mode is OFF → allow centering
  if (!obstacleMode) {
    servoScanning = false;
    scanState = SCAN_IDLE;
    return;
  ...
  servoScanning = true;   // scanning is active
  ...
  switch (scanState) {
    ...
    case SCAN_LEFT:
      if (now - scanTimer > 250) {
        scanServo.write(30);
        scanTimer = now;
        scanState = SCAN_CENTER;
      }
      break;

    case SCAN_CENTER:
      if (now - scanTimer > 250) {
        distLeft = readDistanceSafe();
        scanTimer = now;
    ...
    case SCAN_DECIDE:
      if (now - scanTimer > 250) {
        distRight = readDistanceSafe();
        scanServo.write(90);

        if (distLeft > distRight && distLeft > distCenter)
          turnLeftNext = true;
        else if (distRight > distLeft && distRight > distCenter)
          turnLeftNext = false;

        scanState = SCAN_IDLE;
        servoScanning = false;     // scan finished
        autoState = AUTO_TURN;
        autoStateStart = now;
      }
      break;
...
void handleObstacle() {
  ...
  switch (autoState) {
    case AUTO_IDLE: {
  scanAndDecideDirection();
} break;

  case AUTO_FORWARD: {       // ✅ KEEP SERVO CENTERED
  servoScanning = false;   // ✅ NOT scanning
  scanState = SCAN_IDLE;
     // ✅ NO SCAN ACTIVE
  ...
  if (now - autoStateStart >= TURN_TIME) {
    driveStop();
    scanServo.write(90);        // ✅ FORCE CENTER
    servoScanning = false;      // ✅ scan finished
    scanState = SCAN_IDLE;
...
void handleMode() {
  ...
  scanState = SCAN_IDLE;
  servoScanning = false;     // ✅ RELEASE SERVO
  scanServo.write(90);       // ✅ FORCE CENTER
...
void loop() {
  ...
  // obstacle mode OFF → force servo center
